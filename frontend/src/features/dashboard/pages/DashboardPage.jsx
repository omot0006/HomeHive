...
import useHive from "../../households/hooks/useHive";
...
function Dashboard() {
  const { user } = useAuth();
  const friendlyName = getFriendlyName(user) || "there";
  const userInitials = getUserInitials(user);
  const { hive, isLoading: isHiveLoading, error: hiveError } = useHive();
  const hiveName = hive?.name ?? "Your Hive";
  const ownerName = getFriendlyName(hive?.owner) || "Hive owner";
  ...
  return (
    <div className="min-h-full min-w-0 bg-hive-canvas text-hive-ink">
      ...
      <main className="min-w-0 px-4 py-5 sm:px-6 lg:px-8 lg:py-7 xl:px-10">
        <header className="flex items-center justify-between gap-4"><div><Logo size="small" className="lg:hidden" textClassName="text-hive-ink" /><p className="hidden text-xs font-bold uppercase tracking-[0.18em] text-hive-terracotta lg:block">{hiveName} · Thursday, July 10</p><h1 className="mt-2 text-2xl font-bold tracking-tight sm:text-3xl lg:mt-3 lg:text-4xl">Good morning, {friendlyName}</h1><p className="mt-1 text-sm text-hive-muted sm:text-base">Your household is moving in a good rhythm today.</p></div><div className="flex items-center gap-2"><div className="hidden w-64 xl:block"><div className="flex h-11 items-center gap-2 rounded-hive-md border border-hive-border bg-hive-surface px-3 text-hive-muted shadow-sm"><Search size={17} /><input aria-label="Search your household…" placeholder="Search your household…" className="min-w-0 flex-1 bg-transparent text-sm outline-none placeholder:text-hive-muted/70" /></div></div><IconButton label="Search HomeHive" className="xl:hidden" onClick={() => setIsSearchOpen((open) => !open)}><Search size={19} /></IconButton><div className="relative"><IconButton label="Open notifications" onClick={() => setIsNotificationsOpen((open) => !open)} className="relative"><Bell size={19} /><span className="absolute right-2.5 top-2.5 h-2 w-2 rounded-full bg-hive-terracotta" /></IconButton>{isNotificationsOpen && <Card className="absolute right-0 z-30 mt-3 w-80 p-5"><div className="flex items-center justify-between"><p className="font-bold">Notifications</p><a href="/notifications" className="text-sm font-bold text-hive-terracotta">View all</a></div><div className="mt-4 space-y-3 text-sm text-hive-muted"><p><strong className="text-hive-ink">Chore reminder:</strong> Kitchen reset is due today.</p><p><strong className="text-hive-ink">Rent reminder:</strong> 4 days remaining.</p><p><strong className="text-hive-ink">Birthday ahead:</strong> Sarah&apos;s dinner is Friday.</p></div></Card>}</div><a href="/profile" aria-label={`Open ${friendlyName}'s profile`} className="flex h-12 w-12 items-center justify-center overflow-hidden rounded-full border-2 border-hive-surface bg-hive-terracotta text-sm font-bold text-white shadow-hive-card ring-1 ring-hive-border transition hover:scale-105 focus-visible:outline-none focus-visible:ring-4 focus-visible:ring-[var(--hh-focus-ring)]">{userInitials}</a></div></header>
        ...
        {hiveError && <p role="alert" className="mt-5 rounded-hive-md bg-hive-rose-soft px-4 py-3 text-sm text-hive-terracotta-strong">{hiveError}</p>}
        <Card className="mt-5 p-5 sm:p-6">
          <div className="flex flex-col justify-between gap-5 sm:flex-row sm:items-center">
            <div><p className="text-sm font-bold uppercase tracking-[0.14em] text-hive-terracotta">Your Hive</p><h2 className="mt-2 text-2xl font-bold">{isHiveLoading ? "Loading your Hive…" : hiveName}</h2><p className="mt-1 text-sm text-hive-muted">{hive?.householdType ?? "Household"}{hive?.description ? ` · ${hive.description}` : ""}</p></div>
            <div className="grid grid-cols-3 gap-3 text-center sm:min-w-96"><div className="rounded-hive-md bg-hive-canvas p-3"><p className="text-xs font-bold uppercase tracking-[0.1em] text-hive-muted">Owner</p><p className="mt-2 truncate font-semibold">{ownerName}</p></div><div className="rounded-hive-md bg-hive-canvas p-3"><p className="text-xs font-bold uppercase tracking-[0.1em] text-hive-muted">Members</p><p className="mt-2 font-semibold">{memberCount}</p></div><div className="rounded-hive-md bg-hive-canvas p-3"><p className="text-xs font-bold uppercase tracking-[0.1em] text-hive-muted">Invite code</p><p className="mt-2 font-mono font-bold tracking-wider">{hive?.inviteCode ?? "———"}</p></div></div>
          </div>
        </Card>
